...
class LiberoInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        base_image = _parse_image(data["observation/image"])  # 主摄像头图像
        wrist_image = _parse_image(data["observation/wrist_image"])  # 腕部摄像头图像

        # 创建输入字典。不要更改下面字典中的键名
        inputs = {
            "state": data["observation/state"],  # 机器人状态
            "image": {
                "base_0_rgb": base_image,  # 主摄像头图像
                "left_wrist_0_rgb": wrist_image,  # 左腕摄像头图像
                # 用适当形状的零数组填充任何不存在的图像
