...
def main(args: Args) -> None:
    ...
    # 发送几个观察数据以确保模型已加载
    action_list = []
    for time_step in range(109):

        obs = obs_fn(use_train_obs=True, time_step=time_step)
    ...
    pickle.dump(action_list, open("action_list.pkl", "wb"))
    import pdb; pdb.set_trace()
...
def _random_observation_agi(use_train_obs: bool = False, time_step: int = 0) -> dict:
    ...
    """
    if use_train_obs:
        train_obs = pickle.load(open("/home/tione/notebook/workspace/rickyyzliu/code/convert/train_obs_109.pkl", "rb"))
    else:
